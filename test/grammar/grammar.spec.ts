...
describe('examples from paper', () => {
        ...
        const a = (t:string) =>!!t.match(/a/i);
        const S = "S";
        
        const S2a = {left: S, right:[a],probability:p};
        const S2SS = {left: S, right:[S,S],probability:q};
        
        const grammar:Grammar<string, number> = Grammar.builder("simple-recursive-grammar")
                .addRule(S2a)
                .addRule(S2SS)
        .build();
        ...
        const [chart, i, init] = parseSentenceIntoChart(S, grammar, tokens);
        ...
        const s02SSS1 = chart.states.getState(S2SS, 2, 0, 1);
        ...
        const s02S1 = chart.states.getState(init.rule, 2, 0, 1);
        ...
        const s13S2 = chart.states.getState(S2SS, 3, 1, 2);
        ...
        const s13S1 = chart.states.getState(S2SS, 3, 1, 1);
        ...
        const s03S2 = chart.states.getState(S2SS, 3, 0, 2);
        ...
        const s03S1 = chart.states.getState(S2SS, 3, 0, 1);

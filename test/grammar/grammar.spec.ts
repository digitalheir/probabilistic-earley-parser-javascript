import { parseSentenceIntoChart, getViterbiParseFromChart } from "../../src";
...
describe('examples from paper', () => {
    ...
    const alpha = chart.getForwardScore.bind(chart);
    const gamma = chart.getInnerScore.bind(chart);

    it('State set 0', () => {
        const s00Sa = chart.getState(S2a, 0, 0, 0);

        expect(prob(alpha(s00Sa))).to.equal(1.0);
        expect(prob(gamma(s00Sa))).to.equal(p);


        const s00SSS = chart.getState(S2SS, 0, 0, 0);

        expect(prob(alpha(s00SSS))).to.equal(q / p);
        expect(prob(gamma(s00SSS))).to.equal(q);
    ...
    it('State set 1', () => {
        ...
        expect(prob(alpha(s01Sa1))).to.equal(1);
        expect(prob(gamma(s01Sa1))).to.equal(p);

        // completed
        const s01SSS1 = chart.getState(S2SS, 1, 0, 1);

        expect(prob(alpha(s01SSS1))).to.equal(q);
        expect(prob(gamma(s01SSS1))).to.be.above((p * q) - 0.000001).and.below((p * q) + 0.000001);

        // predicted
        const s11Sa0 = chart.getState(S2a, 1, 1, 0);
        expect(prob(alpha(s11Sa0))).to.equal(q);
        expect(prob(gamma(s11Sa0))).to.equal(p);

        const s11SSS0 = chart.getState(S2SS, 1, 1, 0);
        expect(prob(alpha(s11SSS0))).to.be.above((Math.pow(q, 2) / p) - 0.0001).and.below((Math.pow(q, 2) / p) + 0.0001);
        expect(prob(gamma(s11SSS0))).to.equal(q);

    });
    it('State set 2', () => {
        // scanned
        const s12Sa1 = chart.getState(S2a, 2, 1, 1);
        expect(prob(alpha(s12Sa1))).to.equal(q);
        expect(prob(gamma(s12Sa1))).to.equal(p);

        // completed
        const s12SSS1 = chart.getState(S2SS, 2, 1, 1);
        expect(prob(alpha(s12SSS1))).to.equal(q * q);
        expect(prob(gamma(s12SSS1))).to.be.above((p * q) - 0.000001).and.below((p * q) + 0.000001);

        const s02SSS2 = chart.getState(S2SS, 2, 0, 2);
        expect(prob(alpha(s02SSS2))).to.be.above((p * q) - 0.000001).and.below((p * q) + 0.000001);
        expect(prob(gamma(s02SSS2))).to.be.above((p * p * q) - 0.000001).and.below((p * p * q) + 0.000001);

        const s02SSS1 = chart.getState(S2SS, 2, 0, 1);
        expect(prob(alpha(s02SSS1))).to.be.above((p * q * q) - 0.0001).and.below(((p * q * q) + 0.0001));
        expect(prob(gamma(s02SSS1))).to.be.above((p * p * q * q) - 0.0001).and.below(((p * p * q * q) + 0.0001));

        const s02S1 = chart.getState(init.rule, 2, 0, 1);
        expect(prob(alpha(s02S1))).to.be.above((p * p * q) - 0.0001).and.below(((p * p * q) + 0.0001));
        expect(prob(gamma(s02S1))).to.be.above((p * p * q) - 0.0001).and.below(((p * p * q) + 0.0001));

        // predicted
        const s22S0 = chart.getState(S2a, 2, 2, 0);

        expect(prob(gamma(s22S0))).to.equal(p);
        expect(prob(alpha(s22S0))).to.be.above(((1 + p) * q * q) - 0.00000001).and.below(((1 + p) * q * q) + 0.000000000000001);

        const s22SS0 = chart.getState(S2SS, 2, 2, 0);
        expect(prob(alpha(s22SS0))).to.be.above(((1 + (1 / p)) * q * q * q) - 0.0001).and.below(((1 + 1 / p) * q * q * q) + 0.0001);
        expect(prob(gamma(s22SS0))).to.equal(q);

    });
    it('State set 3', () => {
        // scanned
        const s23Sa1 = chart.getState(S2a, 3, 2, 1);
        expect(prob(alpha(s23Sa1))).to.be.below(((1 + p) * q * q) + 0.0001).and.above(((1 + p) * q * q) - 0.000001);
        expect(prob(gamma(s23Sa1))).to.equal(p);

        // completed
        const s23S1 = chart.getState(S2SS, 3, 2, 1);
        expect(prob(alpha(s23S1))).to.be.below(((1 + p) * q * q * q) + 0.0001).and.above(((1 + p) * q * q * q) - 0.0001);
        expect(prob(gamma(s23S1))).to.be.below((p * q) + 0.0001).and.above((p * q) - 0.0001);

        const s13S2 = chart.getState(S2SS, 3, 1, 2);
        expect(prob(alpha(s13S2))).to.be.below((p * q * q) + 0.0001).and.above((p * q * q) - 0.0001);
        expect(prob(gamma(s13S2))).to.be.below((p * p * q) + 0.0001).and.above((p * p * q) - 0.0001);

        const s13S1 = chart.getState(S2SS, 3, 1, 1);
        expect(prob(alpha(s13S1))).to.be.above((p * q * q * q) - 0.0001).and.below((p * q * q * q) + 0.0001);
        expect(prob(gamma(s13S1))).to.be.above((p * p * q * q) - 0.0001).and.below((p * p * q * q) + 0.0001);

        const s03S2 = chart.getState(S2SS, 3, 0, 2);
        expect(prob(alpha(s03S2))).to.be.above((2 * p * p * q * q) - 0.0001).and.below(((2 * p * p * q * q) + 0.0001));
        expect(prob(gamma(s03S2))).to.be.above((2 * p * p * p * q * q) - 0.0001).and.below(((2 * p * p * p * q * q) + 0.0001));

        const s03S1 = chart.getState(S2SS, 3, 0, 1);
        expect(prob(alpha(s03S1))).to.be.above((2 * p * p * q * q * q) - 0.0001).and.below((2 * p * p * q * q * q) + 0.0001);
        expect(prob(gamma(s03S1))).to.be.above((2 * p * p * p * q * q * q) - 0.0001).and.below((2 * p * p * p * q * q * q) + 0.0001);

        expect(prob(alpha(init))).to.be.above((2 * (Math.pow(p, 3) * Math.pow(q, 2))) - 0.0001).and.above((2 * (Math.pow(p, 3) * Math.pow(q, 2))) + 0.0001);
        expect(prob(gamma(init))).to.be.above((2 * (Math.pow(p, 3) * Math.pow(q, 2))) - 0.0001).and.above((2 * (Math.pow(p, 3) * Math.pow(q, 2))) + 0.0001);
